# Intelligent Vendor Recommendation Platform — Backend

FastAPI service that lets an operations team register vendors, post work requirements, and get **AI-ranked vendor recommendations** for each requirement. Built with FastAPI, PostgreSQL (via Neon) + `pgvector`, Redis (local) for background jobs, and Google Gemini for embeddings & structured LLM reports.

> This document covers **just the backend**. For monorepo layout, the web dashboard, and BFF wiring, see the [root README](../../README.md).
...
flowchart LR
    ...
    subgraph Infra["💾 Infrastructure"]
        ...
        FS[/"uploads/ + reports/<br/>local filesystem"/]:::store
    end
...
| deleted_at | timestamptz | soft-delete marker (from `AuditBase`) |
...
- Every table gets a `deleted_at` column via `AuditBase` and all base queries filter it out — deletes are soft.
...
- `deleted_at IS NULL` (never surface soft-deleted vendors)
...
2. **Structured vendor report (`gemini-3.1-flash-lite`, JSON mode)**
   ...
   - Report shape (roughly):
     ...
     ```
   - `temperature=0.2` for stability. The worker writes the report JSON to `reports/<work_requirement_id>/<UTC-timestamp>.json` and also logs it. Production would persist to an `ai_reports` table and expose `GET .../report/{id}`.

Model IDs and embedding dimensions are all in [app/config/settings.py](app/config/settings.py) so they can be overridden via `.env` without touching code.
...
- **Local filesystem storage** for uploaded documents (`uploads/`) and generated reports (`reports/`). Adequate for a single-node MVP demo.
...
- **Local file uploads and reports.** Zero-setup for a demo; will not survive a container restart and provides no MIME/size validation. See *Future scope*.
...
- **Fire-and-forget report worker.** The `POST .../report` endpoint returns `202` but the result is only logged & written to disk. No status endpoint to retrieve the report.
...
# Neon Postgres (with sslmode=require)
...
#    the JSON also lands under apps/backend/reports/<work_req_id>/)
...
8. **AI report retrieval.** `POST .../report` writes JSON to disk and fires-and-forgets. Add persistence (`ai_reports` table) + `GET .../report/{id}` and a job-status endpoint.
